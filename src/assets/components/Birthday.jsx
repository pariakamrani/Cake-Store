...
export default function Birthday({AddTocard}) {
    const[product,setProduct]=useState([])

     useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}api/birthdaycake.json`)
      .then(res => res.json())
  ...
  console.log(product)
    return (
        <>
            <div className='md:mt-50 sm:mt-40 mt-20 '>
                <div className="max-w-7xl mx-auto px-4 py-10">
                    ...
                    {item.title}
                                ...
                                <div className="mt-4 flex gap-3">
                                    <button onClick={()=>AddTocard(item)} className="flex-1 bg-pink-500 hover:bg-pink-600 text-white py-2 rounded-xl text-sm transition">
                                        افزودن به سبد
                                    </button>
